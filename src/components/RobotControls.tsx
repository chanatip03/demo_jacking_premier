  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-4">
      {websocketGroups.map((group) => (
        <section
          key={group.websocket}
          className="flex flex-1 flex-col rounded-2xl border-t-4 border-blue-400 bg-white p-4 shadow-lg"
        >
          ...
          <div className="flex flex-wrap gap-4">
            {group.sections.map((section, sectionIdx) => (
              <div key={`${group.websocket}-${sectionIdx}`} className="mb-4 last:mb-0">
